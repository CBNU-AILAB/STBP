...
class Wu2018(n3ml.network.Network):
    ...
    def update_potential_spike(self, ops, o, potential, spike):
        x = ops(o)
        potential  = potential * Wu2018.decay * (1. - spike) + x
        STBP_spike = n3ml.layer.Wu()
        spike      = STBP_spike(potential)
        return potential, spike

    def forward(self, input):
        layer1_potential = layer1_spike = torch.zeros(self.batch_size, 32, 28, 28).cuda()
        layer2_potential = layer2_spike = torch.zeros(self.batch_size, 32, 14, 14).cuda()

        layer3_potential = layer3_spike            = torch.zeros(self.batch_size, 128).cuda()
        layer4_potential = layer4_spike = sumspike = torch.zeros(self.batch_size, 10).cuda()

        for time in range(self.time_interval):

            o = (input > torch.rand(input.size()).cuda()).float()

            layer1_potential, layer1_spike = self.update_potential_spike(self.conv1, o, layer1_potential, layer1_spike)
            o = self.avgpool(layer1_spike)

            layer2_potential, layer2_spike = self.update_potential_spike(self.conv2, o, layer2_potential, layer2_spike)
            o = self.avgpool(layer2_spike)

            o = o.view(self.batch_size, -1)

            layer3_potential, layer3_spike = self.update_potential_spike(self.fc1, o,            layer3_potential, layer3_spike)
            layer4_potential, layer4_spike = self.update_potential_spike(self.fc2, layer3_spike, layer4_potential, layer4_spike)
            sumspike += layer4_spike

        outputs = sumspike / self.time_interval
