...
class _Wu(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, potential):
        ctx.save_for_backward(potential)
        spike = potential.gt(_Wu.threshold)
        spike = spike.float()
        return spike
    ...
    def backward(ctx, grad_output):
        ...
        upstream_gradient = grad_output.clone()
        surrogate_gradient = abs(potential - _Wu.threshold) < _Wu.alpha/2
        local_gradient = surrogate_gradient.float()
